...
function aaa() {
    ...
    curseforge.getModFiles(Number(ID)).then((files) => {
        for (let i = 0; i < files.length; i++) {
            ...
            if (data.includes(config.ver) || data.includes(config.version) || data.includes("1.16.4") || data.includes("1.16.3") || data.includes("1.16.2") || data.includes("1.16.1")|| data.includes(config.version_Snapshot)) {
